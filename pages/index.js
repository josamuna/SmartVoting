...
export default function Home() {
  ...
  // Load candidate by ID to load picture
  async function loadSingleCandidate(candidateId) {
    try {
      const provider = new ethers.providers.JsonRpcProvider();
      const contract = new ethers.Contract(votingaddress, Voting.abi, provider);
      const data = await contract.candidateInfo(candidateId);

      const item = await Promise.resolve({
        id: data.id.toNumber(), // Convert BigInt to JavaScript Number.
        orderNumber: data.orderNumber.toNumber(), // Convert BigInt to JavaScript Number
        fullname: data.fullname,
        pictureURI: data.pictureURI,
        image: (await axios.get(data.pictureURI)).data.image, // Fetch Candidate picture from the IPFS URI
        user: data.user,
      });
      setCandidateData(item); // Set State data.
      // console.log("SingleItem = ", item);
    } catch (error) {
      console.error(`loadSingleCandidate => ${error}`);
    }
  }
  ...
  return (
    <section className="flex flex-col justify-center">
      ...
      <article className="flex flex-row">
          ...
          <div className="flex flex-col">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                (() => {
                  if (e.target.value > 0) {
                    loadSingleCandidate(e.target.value);
                  }
                })();
              }}
            />
        ...
        <article className="grow mt-4 ml-4 mr-8 border border-orange-200 rounded p-0">
          <div className="flex flex-row justify-center ">
            <img
              src={candidateData.image}
              alt="Candidate Picture"
              className="w-72 h-72 my-16"
            />
          </div>
        </article>
      </article>
